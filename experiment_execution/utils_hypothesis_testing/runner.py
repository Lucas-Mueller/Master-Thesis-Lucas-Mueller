...
def _repo_root() -> Path:
    """Best-effort detection of the repository root (parent of 'experiment_execution')."""
    here = Path(__file__).resolve()
    # runner.py -> utils_experiment_execution -> experiment_execution -> repo_root
    for p in [here] + list(here.parents):
        if (p / 'experiment_execution').is_dir() and (p / 'main.py').exists():
            # 'p' could be repo root or experiment_execution; prefer the one with main.py
            if (p / 'main.py').exists():
                return p
    # Fallback to cwd
...
def run_configs_in_parallel(
    ...
    concurrency: int = 2,
    logs_dir: str | Path = "experiment_execution/hypothesis_1/terminal_outputs",
    results_dir: str | Path = "experiment_execution/hypothesis_1/results",
    timeout_sec: Optional[int] = None,
